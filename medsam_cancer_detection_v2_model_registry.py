...
class CancerDetectorBase(nn.Module, ABC):
    ...
    def show_example(self, image, needle_mask, label, prostate_mask=None):
        ...
        output: CancerDetectorOutput = self(
            image, needle_mask, label, prostate_mask=prostate_mask
        )
...
class MedSAMCancerDetectorV2(nn.Module):
    def __init__(
        ...
        if image_encoder_checkpoint is not None:
            self.medsam_model.image_encoder.load_state_dict(
                torch.load(image_encoder_checkpoint, map_location="cpu")
            )
        if freeze_backbone:
...
class MedSAMCancerDetectorWithAdapters(MedSAMCancerDetectorV2):
    def __init__(
        ...
        wrap_image_encoder_with_adapter(
            self.medsam_model.image_encoder,
...
class SlidingWindowHeatmapGenerator(torch.nn.Module):
    def __init__(
        ...
        self.rearrange_patches = Rearrange(
            "b (c n_h n_w) h w -> b c n_h n_w h w",
            n_h=self.n_windows_h,
            n_w=self.n_windows_w,
            h=window_size[0],
            w=window_size[1],
        )
...
class ResnetSlidingWindowCancerDetector:
    def __init__(self, freeze_model=True):
        ...
        self.model = LinearEval(
            feature_extractor, linear_layer, freeze_model=freeze_model
        )
    ...
    def forward(self, image, needle_mask, label, prostate_mask) -> CancerDetectorOutput:
        ...
        with torch.no_grad():
            ...
            needle_mask = torch.nn.functional.interpolate(
                needle_mask, size=(H, W), mode="nearest"
            )
            prostate_mask = torch.nn.functional.interpolate(
                prostate_mask, size=(H, W), mode="nearest"
            )
            ...
            bmode = (bmode - bmode.mean(dim=(-1, -2, -3), keepdim=True)) / bmode.std(
                dim=(-1, -2, -3), keepdim=True
            )
            bmode = torch.nn.functional.interpolate(
                bmode, size=(256, 256), mode="bilinear", align_corners=False
            )
